...
var eor_indy_assembled := PackedByteArray([
	...
	0x5a, 0x8d, 0x01, 0x02, 0xa9, 0x55, 0x51, 0x50, 0x51, 0x52
])
...
func test_lsr_accum_carry():
	...
	cpu.set_flag(CPU.FlagBit.CARRY, false)
	cpu.step()
	assert_int(cpu.A).is_equal(0x3)
	cpu.step()
	assert_flag(CPU.FlagBit.CARRY).is_true()
	cpu.step()
	assert_flag(CPU.FlagBit.CARRY).is_true()
	cpu.step()
	assert_flag(CPU.FlagBit.CARRY).is_false()
	assert_flag(CPU.FlagBit.ZERO).is_true()
...
func test_bit():
	...
	cpu.step(2)
	assert_int(cpu.A).is_equal(0x80)
	assert_bool(cpu.get_flag_state(CPU.FlagBit.NEGATIVE)).is_true()
	assert_bool(cpu.get_flag_state(CPU.FlagBit.OVERFLOW)).is_false()
	cpu.step()
	assert_bool(cpu.get_flag_state(CPU.FlagBit.NEGATIVE)).is_false()
	...
	assert_bool(cpu.get_flag_state(CPU.FlagBit.OVERFLOW)).is_false()
	cpu.step()
	assert_bool(cpu.get_flag_state(CPU.FlagBit.OVERFLOW)).is_true()
	cpu.step(3)
	assert_bool(cpu.get_flag_state(CPU.FlagBit.NEGATIVE)).is_false()
	assert_bool(cpu.get_flag_state(CPU.FlagBit.OVERFLOW)).is_true()
	cpu.step()
	...
	assert_bool(cpu.get_flag_state(CPU.FlagBit.ZERO)).is_false()
	cpu.step()
	assert_bool(cpu.get_flag_state(CPU.FlagBit.ZERO)).is_false()
